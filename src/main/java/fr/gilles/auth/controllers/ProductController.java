...
public class ProductController {
    ...
    @Operation(summary = "Search product by name | ADMIN OR MANAGER AUTHORIZATION", security = @SecurityRequirement(name = "Bearer Token"))
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_MANAGER')")
    public ResponseEntity<Page<Product>> searchByName(@PathVariable @NotNull @NotEmpty @NotBlank String name, QueryParams queryParams, Authentication authentication) {
        Admin admin = (Admin) userService.findByEmail(authentication.getName());
        if (admin != null) {
            if (admin.getRoles().stream().anyMatch(r -> r.getName().equals("ROLE_ADMIN"))) {
